...
int main() {
    ...
    size_t RBytes = 2 * N * sizeof(float);  // reading A and B
    size_t WBytes = N * sizeof(float);     // writing C
    float timeSeconds = milliseconds / 1000.0f;

    float measuredBW = (RBytes + WBytes) / (timeSeconds * 1e9);  // bytes to GB
    cout << "Measured Memory Bandwidth: " << measuredBW << " GB/s\n";
